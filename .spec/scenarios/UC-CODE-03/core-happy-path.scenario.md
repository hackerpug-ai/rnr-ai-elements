...
end_state:
  must_observe: ["Developer can render a live web preview inside a native webview with a visible URL bar and a reload control."]
...
---
# Live web preview through a native webview — core flow

The web-preview surface is delivered as a native webview with an explicit dependency contract and a real error state.

Verify on an iOS simulator AND an Android emulator: Developer can render a live web preview inside a native webview with a visible URL bar and a reload control.
